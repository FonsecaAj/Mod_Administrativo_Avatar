    ...
    public class ModuloEditarModel : BasePageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                {
                    TempData["Mensaje"] = $"Módulo actualizado exitosamente a '{moduloActualizado.Nombre}'";
                    return RedirectToPage("/Modulo/Modulos");
                }

                ModelState.AddModelError(string.Empty, "Error al actualizar el módulo. Verifique que el nombre no esté registrado.");
                NombreOriginal = moduloOriginal.Nombre;
                return Page();
            }
            catch (Exception ex)
