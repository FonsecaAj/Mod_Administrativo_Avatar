    ...
    public class IndexModel : BasePageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                ...
                _logger.LogInformation("Dashboard cargado para: {Email} con rol: {Rol}",
                    UsuarioEmail, UsuarioRol);
        ...
        private async Task RegistrarVisualizacionDashboardAsync(string token)
        {
            try
            {
                ...
                if (registrado)
                {
                    _logger.LogDebug("Visualización del dashboard registrada en bitácora para {Email}",
                        UsuarioEmail);
                }
                else
                {
                    _logger.LogWarning("No se pudo registrar la visualización del dashboard en bitácora para {Email}",
                        UsuarioEmail);
                }
            }
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(ex, "Error al registrar visualización del dashboard en bitácora para {Email}",
                    UsuarioEmail);
            }
        }
